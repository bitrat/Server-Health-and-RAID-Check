...
$ReportName = "ABC TECH SERVER  - "+$ComputerName+" -  System Report - LSI RAID Controller - "+$now.Day+" "+$now.Month+" "+$now.Year
...
$ReportBody += Get-HtmlContentOpen -HeaderText "System Report For - $ComputerName"	 -BackgroundShade 2
    ...
    #Region Gallagher Backup and Archives report
     ...
     # Gallagher Archives
       ...
       <# Disable for Win Server versions - only works on Win 7 to Win 10
        ...
        Try
        {
                ...
                $ReportBody += Get-HtmlContentOpen -HeaderText "RAID Error Events Log - PLEASE INVESTIGATE any RAID ERRORS Listed Below" #-IsHidden
                    ...
                    foreach ($EventID in $EventIDs)
                    {
                       ...
                       #$RAIDEvent = Get-EventLog $EventLogName | Where-Object {$_.EventID -eq $EventID} | Select-Object -Property EventID,EventType,Message
                       # Make SURE - You have taken this script OFF TEST (Test EventIDs) !!!
