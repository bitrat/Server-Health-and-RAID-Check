...
if($LSI){ 
    ...
    # Appends by default
    Get-Content C:\HealthCheck\EventLog_EventIDs\LSI_MR_Monitor_RAID_Events_FATAL.txt | Add-Content C:\HealthCheck\RAID\LSI_RAID_EventIDs.txt
    Get-Content C:\HealthCheck\EventLog_EventIDs\LSI_MR_Monitor_RAID_Events_CRITICAL.txt | Add-Content C:\HealthCheck\RAID\LSI_RAID_EventIDs.txt
    Get-Content C:\HealthCheck\EventLog_EventIDs\LSI_MR_Monitor_RAID_Events_ERROR.txt | Add-Content C:\HealthCheck\RAID\LSI_RAID_EventIDs.txt

    }
    else {
        # ALL Errors checked = Fatal, Error, Critical, Warning
        Get-Content C:\HealthCheck\EventLog_EventIDs\LSI*.txt | Add-Content C:\HealthCheck\RAID\LSI_RAID_EventIDs.txt
    }
 }
...
elseif($P440ar){
    ...
    if($HIGH){
        ...
        # Appends by default
        Get-Content C:\HealthCheck\EventLog_EventIDs\HPE_SmartArray_P440ar_ERROR.txt | Add-Content C:\HealthCheck\RAID\P440ar_RAID_EventIDs.txt
    }
    else {

        # ALL Errors checked = Fatal, Error, Critical, Warning
        Get-Content C:\HealthCheck\EventLog_EventIDs\HPE*.txt | Add-Content C:\HealthCheck\RAID\P440ar_RAID_EventIDs.txt
       }
 }
else {
